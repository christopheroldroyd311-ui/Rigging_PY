...
#     if not selection:
#         cmds.error("Please select one or more meshes first to paint.")
#         return

#     skeleton_root = "c__waist__JNT__BIND"

#     if not cmds.objExists(skeleton_root):
#         cmds.error(f"Skeleton root '{skeleton_root}' not found. Please check your joint name.")
#         return

#     skin_clusters = []

#     for skinned_mesh in selection:
    
#         shapes = cmds.listRelatives(skinned_mesh, shapes=True, type='mesh') or []
#         if not shapes:
#             cmds.warning(f"Skipping {skinned_mesh} — no mesh shape found.")
#             continue
...
#             print(f" Bound {skinned_mesh} to {skeleton_root} with skin cluster {skin_cluster}")
#         except Exception as e:
#             cmds.warning(f" Could not bind {skinned_mesh}")

#     if not skin_clusters:
#         cmds.error("No meshes were successfully skinned.")
#         return

#     mel.eval('artPaintSkinWeightsTool;')

#     cmds.select(selection[-1])
#     print(f"Opened Paint Skin Weights Tool for {selection[-1]}")
